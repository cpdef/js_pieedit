...
for (var i=0; i < lines.length; i++)
{
  ...
  } else if (block === "ANIMOBJECT")
  {
    if (cols[0] === "ANIMOBJECT")
    {
      ...
      frame = {
        pos: {
          x: Number(cols[1] / 1000),
          y: Number(cols[2] / 1000),
          z: Number(cols[3] / 1000)
        },
        rot: {
          x: Number(cols[4])*TO_DEG_FACTOR,
          y: Number(cols[5])*TO_DEG_FACTOR,
          z: Number(cols[6])*TO_DEG_FACTOR
        },
        scale: {
          x: Number(cols[7]),
          y: Number(cols[8]),
      ...
      LEVELS[LEVEL].animframes.push(frame);
      animframe++;
